<?php
if (class_exists('ProsociateProductImport') != true) {
    class ProsociateProductImport
    {
     ...
     * @var string
     */
    private static $option = 'postNewProduct';
     ...
     * @var string
     */
    private static $optionAsins = 'prosAsins';
    ...
    private $items = null;
     /**
     * externalProductUrl for product
     *
     * @var string
     */
    ...
    */
    public function __construct()
    {
		...
		if (is_admin()) {
            ...
            if ( isset($_GET['page']) && $_GET['page'] == 'pros-products-import' ){
				add_action('admin_enqueue_scripts', array($this, 'admin_enqueue_enhanncde_script'));
            }
		}
		        // Get status
        //$status = get_option(self::$option, 'postNewProduct');

        //$this->resetOptions();
        // Check status
		//add_action('show_import_log', array($this, 'captureFileRequest'));
        add_action('wp_ajax_prossociate_iterate_import', array($this, 'start_import_asins'));
		add_action('admin_init', array($this, 'captureFileRequest'));

    }

     public function admin_enqueue_enhanncde_script(){
	    	wp_dequeue_script('prossociate_poster');
	    	wp_register_script('my-jquery-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/1.8.2/jquery-ui.js');
        ...
        wp_enqueue_style('jquery-ui-gapi');
	    	wp_enqueue_script('prossociate_stats', PROSSOCIATE_ROOT_URL.'/js/ProssociateEnhanced.js', array(), '1.0');
            wp_register_style('pros_admin_style', PROSSOCIATE_ROOT_URL.'/css/admin_style.css');
            wp_enqueue_style('pros_admin_style');



	    	//wp_enqueue_script('prossociate_stats');
	    }
    ...
    */
    public function captureFileRequest() {

        if(isset($_POST['csv_upload']) && check_admin_referer( 'upload_csv_file', 'csv_upload_nonce' ) ) {
          ini_set("auto_detect_line_endings", "1");
            //$target_file = "/home/ajazali/products.csv";
            $file = $_FILES['asin_csv'];
            $upload_dir = wp_upload_dir();
            $target_file_path = $upload_dir['basedir'].'/'.basename($_FILES["asin_csv"]["name"]);
            //$upload = wp_handle_upload($file, array('test_form' => false));
            $uploaded = move_uploaded_file($_FILES["asin_csv"]["tmp_name"], $target_file_path);

            include_once(PROSSOCIATE_ROOT_DIR . '/libraries/parsecsv.lib.php');
            $csv = new parseCSV();
            //iconv( "Windows-1252", "UTF-8", $str );
            //$csv->encoding('UTF-8', "UTF-8");

            if(isset($_POST['encoding']) && ! empty($_POST['encoding'])){
                //$csv->encoding('UTF-16', 'UTF-8', 'ISO-8859-1');
                $csv->encoding($_POST['encoding']);
            }

            //$csv->delimiter = "\t";
            $csv->parse($target_file_path);
            $data = $csv->data;
            $total_asins = count($data);
            //$serAsins  = serialize($data);
            $_POST_saved = update_option('post_options', $_POST);
            $total_asins_saved = update_option('total_asins', $total_asins);
            $prosAsins_saved = update_option('prosAsins', $data);
    ...
    function start_import_asins(){
            $this->post_options = get_option('post_options');
    ...
    private function checkStatus($status) {
        ...
        switch($status) {
            ...
            case 'postNewProduct':
                ...
                $total_asins = get_option('total_asins');
    	           $serAsinsOption = get_option('prosAsins');

                //$asins = $serAsinsOption[0]['ASINS'];

                $update_offset = $var_offset+10;
                $asins = $serAsinsOption[$var_offset]['ASINS'];
                for ($i = $var_offset+1; $i < $update_offset; $i++) {
                   if($serAsinsOption[$i]['ASINS'] != '')
                   $asins .=','.$serAsinsOption[$i]['ASINS'];
                }
                $var_offset = $page*10;
                //echo 'memory before amazon:'. memory_get_usage().'<br>';
                $items_data = new ProssociateItemMultiple($asins);
                //echo 'memory amazon:'. memory_get_usage().'<br>';


                foreach ($items_data->data as $item) {

                   $this->items[$item->ASIN] = $item;
//var_dump($this->items[$item->ASIN]);
                   $asin = $item->ASIN;

                    //$asin = iconv('UTF-16LE', 'UTF-8', $asin);
                    if(!$this->isAsinExist($asin)){
                        echo 'Posting data for '.$asin.'<br>';
                       $postId = $this->postNewProduct($asin);
                       if($postId)
                           echo ' Posted '.$asin.' with post id: '.$postId.'<br>';
                       else
                           echo ' Failed to post '.$asin.'<br>';

                    }
                    else if($this->isAsinExist($asin)){
                        echo 'Product '.$asin.' already exists<br>';
                    }
                    ...
                    if($this->isVariation) {
                        ...
                        $this->postNewVariation();
                        //break;
                    }
                    $global_counter++;
                 }
                 unset($items_data);
                 //echo 'memory after amazon:'. memory_get_usage().'<br>';
                 $page++;
                    //sleep(1);
                ...
                $floor_total_asins = ceil($total_asins/10);
                $complete = ( $floor_total_asins+1 == $page )? true : false;
                if($i >= $update_offset){
     ...
     * @return int
     */
     private function postNewVariation(){
    ...
    private function post($item) {
        ...
        // Default: false
        if(!(isset($this->post_options['postchild']) && ($this->post_options['postchild'] == 'postchild'))) {
        ...
        if((!isset($data->Offers) || $data->Offers->TotalOffers === 0) && !isset($data->Variations)) {
            ...
            if($dmAutoAffiliate == 'true')
                $external = true;
            elseif($this->post_options['externalaffilate'] == 'affiliate') {
                $external = true;
            }
        }
        ...
        if(!$external) {
			if(isset($data->Variations) && $data->Variations->TotalVariations > 0) {
                ...
                if($finalPrice == 'Too low to display' || $finalPrice == '') {
					...
					if(!isset($this->post_options['postfree'])) {
            ...
            } elseif($finalPrice === 'Too low to display' || $finalPrice == '' || $finalPrice == 0) {
                // Check option if we will not post products without prices
                if(!isset($this->post_options['postfree'])) {
                    return false;
                }
        ...
        $trimmedTitle = wordwrap($data->ItemAttributes->Title, $titleLength, "dmpros123", false);
        ...
        if(isset($this->post_options['post_options']['draft']) && $this->post_options['post_options']['draft'] == 'draft') {
            $dmPostStatus = 'draft';
        } else {
        ...
        if( (isset($this->post_options['enablespin']) && ($this->post_options['enablespin'] == 'enablespin')) || $sr_enabled == "true" ) {
        ...
        //if ($post_options['post_options']['download_images'] == 'on') {
            $this->set_post_images($data, $post_id, $dmIsVariation);
        //}
        // Check if there are variations

        //var_dump($data->Variations);
        if (isset($data->Variations) && $post_options['post_options']['externalaffilate'] == 'simple') {
            // Product is a variation
            $this->isVariation = true;

        } elseif (isset($data->Variations) && $post_options['post_options']['externalaffilate'] == 'affiliate') {
            ...
            // Get the sale price
            $finalSaleAmount = 0;
            //var_dump($data->VariationSummary);
        ...
        if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
            ...
            if ($post_options['post_options']['post_type'] == 'product') {
            ...
            if ($post_options['post_options']['externalaffilate'] == 'affiliate' || $external === true) {
                // Set as external
        ...
        if ($post_options['post_options']['auto_category'] == 'yes') {
        ...
        if(isset($post_options['post_options']['dm_select_category'])) {
            if ($post_options['post_options']['dm_select_category'] == 'yes') {
                $forcedAssignedCats = $post_options['tax_input']['product_cat'];
...
die();*/
        ...
        if(($external || $post_options['post_options']['externalaffilate'] == 'affiliate') && !empty($finalListPrice) && ($finalListAmount != 0)) {
...
/*            echo ':Variations';
 var_dump($data->Variations);*/
    ...
    /**
	    * Singleton pattern
	    *
	    * @return ProsociateProductImport Singleton instance
	    */
	    ...
	    public function display_product_import()
	    {  //echo PROSSOCIATE_ROOT_DIR . '/modules/product_stats/ProductStatsView.php';
	       include_once(PROSSOCIATE_ROOT_DIR . '/modules/product_import/ProductImportView.php');
	    }
...
AND {$wpdb->term_taxonomy}.taxonomy = '{$taxonomy}'";
                    ...
                    foreach ($data->ImageSets->ImageSet as $k => $i) {
                        ...
                        if ($k == 0) {
                            ...
                            if(isset($data->LargeImage->URL)){
                              ...
                              $attach_id_small = $this->set_post_featured_thumb($data->LargeImage->URL, $data->ItemAttributes->Title, $post_id);
                              //$att = wp_get_attachment_url( $attach_id_small);
                               //$image = wp_get_attachment_image( $attach_id_small, 'thumbnail' );
                              $set = set_post_thumbnail($post_id, $attach_id_small);
    ...
    private function wordpressSeobyYoastIntegration($post_id, $finalTitle) {
        ...
        if(empty($seoDesc) || $seoDesc === false) {
            // Get the description
            $description = '';
            $EditorialReviews = get_post_meta($post_id, '_pros_EditorialReviews', true);
