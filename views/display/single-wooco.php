...
if($altDesc == false || empty($altDesc)) {
    if ($EditorialReviews->EditorialReview) {
        ...
        if (count($EditorialReviews->EditorialReview) == 1) {
            ...
            if ($EditorialReviews->EditorialReview->Source != "Product Description") {
                /*$spinned_source = prosociate_cprof_rewrite($EditorialReviews->EditorialReview->Source);
                echo "<b>" . $spinned_source . "</b>:single:<br />";*/
                echo "<b>" . $EditorialReviews->EditorialReview->Source . "</b><br />";
            }
            //$spinned_content = prosociate_cprof_rewrite($EditorialReviews->EditorialReview->Content);
            //  echo "<b>" . $spinned_content . "</b>:single-content:<br />";
    ...
    echo "<p class='pros_product_description'>";
    echo html_entity_decode($altDesc, ENT_QUOTES, "UTF-8");
    echo "</p>";
}