jQuery(document).ready(function($) {
    // Hide the geo target notice if Post Product As = External / Affiliate
    $(".dmExtAff").change(function(){
    ...
    $("#tabs-general-settings-link").click(function(){
        ...
        $("#tabs-requirements-settings").hide();
        ...
        $("#tabs-requirements-settings-link").removeClass("nav-tab-active");
    });
    ...
    $("#tabs-requirements-link").click(function(){
        $("#tabs-compliance-settings").hide();
        $("#tabs-general-settings").hide();
        $("#tabs-requirements-settings").show();
        $("#tabs-requirements-link").addClass("nav-tab-active");
        $("#tabs-compliance-settings-link").removeClass("nav-tab-active");
        $("#tabs-general-settings-link").removeClass("nav-tab-active");
    });
    ...
    // DM - new dialog for browsenode
    // If the browse node manually radio button is clicked
    $('#dmCatManual').change(function(){
        dmNodeChoice('manual', this.checked)
    });

    $('#dmCatNormal').change(function(){
        dmNodeChoice('tree', this.checked)
    });

    /*
    $('#spanManual').click(function(){
        $('#dmCatManual').prop("checked", true);
        dmNodeChoice('manual', $('#dmCatManual').is(':checked'))
    });
    */

    /*
    $('#spanTree').click(function(){
        $('#dmCatNormal').prop("checked", true);
        dmNodeChoice('tree', $('#dmCatNormal').is(':checked'))
    });
    */

    function dmNodeChoice(choice, isChecked) {
        if(choice === 'manual') {
            if(isChecked) {
                var e = $('#dmBrowseNodeText');
                e[0].disabled = false;
                $("#jstree_upper_container").hide();
                document.getElementById('dmnode').value = 'manual';
            }
        } else if(choice === 'tree') {
            if(isChecked) {
                var e = $('#dmBrowseNodeText');
                e[0].disabled = true;
                $("#jstree_upper_container").show();
                document.getElementById('dmnode').value = 'tree';
            }
        }
    }
    ...
    $('#dmAutoAffiliate').change(function(){
        var dmAutoAffiliate = this.checked;
        if(dmAutoAffiliate) {
            $('#dmAutoAffiliateCheck').val('true');
        } else {
            $('#dmAutoAffiliateCheck').val('');
        }
    });
    ...
    $('#pros_search_button').click(function() {
        ...
        var dmAsinListsCheck = document.getElementById('dmasinlists').value;
        ...
        if(document.getElementById("dmCatManual") != null) {
            if(document.getElementById("dmCatManual").checked) {
                document.getElementById("browsenode").value = document.getElementById("dmBrowseNodeText").value;
            }
        }
    ...
    function prossociate_search( testHolder ) {
        ...
        var data = {
            ...
            sortby: $('#sortby').val(),
            minprice: $('#dmminprice').val(),
            maxprice: $('#dmmaxprice').val(),
            availability: $('#availability').val(),
            condition: $('#condition').val(),
            manufacturer: $('#manufacturer').val(),
            brand: $('#brand').val(),
            merchantid: $('#dmAmazonOnlyCheck').val(),
            minpercentageoff: $('#minpercentageoff').val(),
            merchantid: $('#merchantid').val(),
            page: 1,
            addAttributes: $('#dmAdditionalAttributesCheck').val(),
            dmasinlists: $('#dmasinlists').val().replace("\n",","),
    ...
    function prossociate() {
        ...
        $('.pros_page_link').click(function(e) {
            ...
            $.post(ajaxurl, data, function(response) {
                ...
                if(data.campaign_type === 'subscription') {
                    dmProdCount = 'All currently available results will be posted now. <a id="dm-tooltip" class="help_tip" data-tip="When new products are found for this search, they will be automatically posted." href="#"><img src="../wp-content/plugins/woocommerce/assets/images/help.png" height="16px" width="16px"/></a>';
                } else if( dmProdCount == 0 )
                {
    ...
    // Load edit campaign
    if (($("#searchindex").val() == '' && $('#dmasinlists').val() != '') || $('#dmasinlists').val() == '' && $("#searchindex").val() != '') {
...
function validate_search_options() {
   ...
   //     return;
    ...
    if (minprice != '' && !is_int(minprice, true)) {
        alert('Minimum Price should be positive integer value.');
        return false;
    }

    var maxprice = document.getElementById('maxprice').value;
    if (maxprice != '' && !is_int(maxprice, true)) {
        alert('Maximum Price should be positive integer value.');
        return false;
    }
    */
    ...
    if (availability != '') {
        ...
        if (merchantid != 'Amazon') {
            alert('You should check Amazon Product Only when you search for Availabile Only.');
            return false;
        }

        var condition = document.getElementById('condition').value;
        if (condition == 'New') {
            alert('You can not set condition as New when you search for Availabile Only.');
            return false;
        }
    }

    var minpercentageoff = document.getElementById('minpercentageoff').value;
    if (minpercentageoff != '' && !is_int(minpercentageoff, true)) {
        alert('Min Percentage Off should be positive integer value.');
        return false;
    }
...
function dm_overlay() {
    jQuery('.dm-tab-search').css('opacity', '0.3');
    jQuery('.nav-tab-wrapper').css('opacity', '0.3');
    jQuery('.wrap h2').css('opacity', '0.3');
}

function dm_deoverlay() {
    jQuery('.dm-tab-search').css('opacity', '1');
    jQuery('.nav-tab-wrapper').css('opacity', '1');
    jQuery('.wrap h2').css('opacity', '1');
}
